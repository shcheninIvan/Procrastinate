...
function getRandomElement(arr) {
  let randIndex = Math.floor(Math.random() * arr.length);//округление умноженное на случайный элемент умноженный на длинну массива
...
button.addEventListener('click', function () {//подключение слушателя нажатия клавиш
  let randomElement = getRandomElement(phrases);
  smoothly(phrase, 'textContent', randomElement.text);//использование библиотеки
  smoothly(image, 'src', randomElement.image);

  if (randomElement.text.length > 40) {
    advice.style.fontSize = '33px';//отсылка на ЦСС и редактирование параметра
  } else {
    advice.style.fontSize = '42px';
  }
});
for( let i=0;i<=2;i=i+1){//Цыкл прокрутки первых 3 значений массива
